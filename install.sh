...
mapfile -t layouts < <(ls numpad_layouts)
...
LAYOUT=$model PERCENTAGE_KEY=$percentage_key envsubst "\$LAYOUT \$PERCENTAGE_KEY" <asus_touchpad.service >/etc/systemd/system/asus_touchpad_numpad.service
...
if ! systemctl enable asus_touchpad_numpad; then
...
if ! systemctl restart asus_touchpad_numpad; then
    echo "Something gone wrong while enabling asus_touchpad_numpad.service"
    exit 1
else
